...
if($rcount>0){
	...
	$p0="<img src='../upload/promotions/$p0' width='120'/>"	;
...
$cate="Event";
	...
	$topicTh=strip_tags($topicTh);
			...
			$result.=
				...
				."<td valign='top' style='width:90px'><a href='?p=promotion/form&id=$proid&status=$status'  class='ui-widget-header  ui-corner-all' style='padding:1px;margin'>แก้ไข</a></td></tr></table>";
		$opt="";
		$img="";
		}
	$result.="</div>";
...
echo "<h3><a href='?p=promotion/form&status=$get'>เพิ่มรายการ </a></h3>";
...
<input type="button" name="btn" value="บันทึกข้อมูล" onclick="if(updatepromotion('Promotion','saveform.php',displaymain)==true){alert('บันทึกเรียบร้อยค่ะ');self.location='administrator.php?p=promotion/index&status=<?=$get?>';} "/>
</center>
</form>
</div>