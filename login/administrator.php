<?  session_start();
	...
	if($_GET['logout']=='yes'){
		...
		echo Message1(45,"red","Login again the Link below.","","<a href='index.php'>Login again</a>");
		exit();
	}
	...
	if(($user_wb!=@$config[adminuser]) || ($passwd_wb!=@$config[adminpwd])){
		echo Message1(45,"red","Only Webmasters only","","<a href='index.php'>Login again</a>");
		session_start();
		unset($_SESSION["AdminName"],$_SESSION["AdminPassword"]);
		exit();
	}
...
<div style="min-height:450px;background:#fefefe;margin:10px" id="main"> 
 ...
 ?>
   <h2 style="padding:50px;color:#333">Select the desired top</h2>   
   <? }?>
